...
import org.kkafka.extensions.submitToContextDispatcher
...
public suspend fun <K, V> KafkaProducer<K, V>.sendSuspending(record: ProducerRecord<K, V>): RecordMetadata =
    suspendCoroutine { continuation ->
        send(record) { metadata: RecordMetadata?, exception: Exception? ->
            // This callback is executed in kafka's IO thread so we resume the continuation in this
            // context's CoroutineDispatcher
            continuation.submitToContextDispatcher {
                if (exception != null) failure(exception) else success(metadata!!)
            }
        }
    }
